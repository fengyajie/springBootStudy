...
(function (factory) {
  ...
  var list = (function () {
    ...
    var compact = function (array) {
      var aApiResult = [];
      for (var idx = 0, len = array.length; idx < len; idx ++) {
        if (array[idx]) { aApiResult.push(array[idx]); }
      }
      return aApiResult;
    };
   ...
   * @param {enum} where  Where change will be applied Row or Col. Use enum: TableApiResultAction.where
   * @param {enum} action Action to be applied. Use enum: TableApiResultAction.requestAction
   * @param {object} domTable Dom element of table to make changes.
   */
  var TableApiResultAction = function (startPoint, where, action, domTable) {
     ...
     */
    function getDeleteApiResultActionToCell(cell) {
      switch (where) {
        case TableApiResultAction.where.Column:
          if (cell.isColSpan) {
            return TableApiResultAction.resultAction.SubtractSpanCount;
          }
          break;
        case TableApiResultAction.where.Row:
          if (!cell.isVirtual && cell.isRowSpan) {
            return TableApiResultAction.resultAction.AddCell;
          }
      ...
      return TableApiResultAction.resultAction.RemoveCell;
    }
     ...
     * @param {object} cell virtual table cell to apply action
     */
    function getAddApiResultActionToCell(cell) {
      switch (where) {
        case TableApiResultAction.where.Column:
          if (cell.isColSpan) {
            return TableApiResultAction.resultAction.SumSpanCount;
          } else if (cell.isRowSpan && cell.isVirtual) {
            return TableApiResultAction.resultAction.Ignore;
          }
          break;
        case TableApiResultAction.where.Row:
          if (cell.isRowSpan) {
    ...
    this.getActionList = function () {
      var fixedRow = (where === TableApiResultAction.where.Row) ? _startPoint.rowPos : -1;
      var fixedCol = (where === TableApiResultAction.where.Column) ? _startPoint.colPos : -1;
      ...
      while (canContinue) {
        ...
        var resultAction = TableApiResultAction.resultAction.Ignore;
        switch (action) {
          case TableApiResultAction.requestAction.Add:
            resultAction = getAddApiResultActionToCell(cell);
            break;
          case TableApiResultAction.requestAction.Delete:
            resultAction = getDeleteApiResultActionToCell(cell);
            break;
        }
  ...
  */
  TableApiResultAction.where = { 'Row': 0, 'Column': 1 };
  /**
  * 
  * Requested action to apply enum.
  */
  TableApiResultAction.requestAction = { 'Add': 0, 'Delete': 1 };
  /**
  * 
  * ApiResult action to be executed enum.
  */
  TableApiResultAction.resultAction = { 'Ignore': 0, 'SubtractSpanCount': 1, 'RemoveCell': 2, 'AddCell': 3, 'SumSpanCount': 4 };
  ...
  var Table = function () {
    ...
    this.addRow = function (rng, position) {
      ...
      var vTable = new TableApiResultAction(cell, TableApiResultAction.where.Row,
        TableApiResultAction.requestAction.Add, $(currentTr).closest('table')[0]);
      ...
      for (var idCell = 0; idCell < actions.length; idCell++) {
        ...
        switch (currentCell.action) {
          case TableApiResultAction.resultAction.AddCell:
            html.append('<td' + tdAttributes + '>' + dom.blank + '</td>');
            break;
          case TableApiResultAction.resultAction.SumSpanCount:
            if (position === 'top') {
    ...
    this.addCol = function (rng, position) {
      ...
      var vTable = new TableApiResultAction(cell, TableApiResultAction.where.Column,
        TableApiResultAction.requestAction.Add, $(row).closest('table')[0]);
      var actions = vTable.getActionList();
    ...
    */
    ...
    this.deleteRow = function (rng) {
      ...
      var vTable = new TableApiResultAction(cell, TableApiResultAction.where.Row,
        TableApiResultAction.requestAction.Delete, $(row).closest('table')[0]);
      var actions = vTable.getActionList();
      ...
      for (var actionIndex = 0; actionIndex < actions.length; actionIndex++) {
        ...
        switch (actions[actionIndex].action) {
          case TableApiResultAction.resultAction.Ignore:
            continue;
          case TableApiResultAction.resultAction.AddCell:
            ...
            continue;
          case TableApiResultAction.resultAction.SubtractSpanCount:
            if (hasRowspan) {
              if (rowspanNumber > 2) {
                rowspanNumber--;
            ...
            continue;
          case TableApiResultAction.resultAction.RemoveCell:
