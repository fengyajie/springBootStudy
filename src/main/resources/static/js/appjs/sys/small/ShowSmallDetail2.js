...
function getGridDataColumns() {
	gridDataColumns = [
			...
			{field:'productNum',title:'采购数量',align:'center',
				formatter : function(value, row, index) {
					var html = "<a onclick='showProductDetail(this)' type='3'repTime='"+row.repTime+"' businessId=" + "'" +row.businessId+"'>"+value+"</a>";
					return html;
				},
			},
			{field:'allowanceAmount',title:'使用抵用金',align:'center'},
			{field:'amount',title:'实付金额',align:'center'},
...
function showProductDetail(currThis) {
	var type = $(currThis).attr('type');
	var businessId = $(currThis).attr('businessId');
	var repTime = $(currThis).attr('repTime');
	var str = type + "," + businessId + "," + repTime;
